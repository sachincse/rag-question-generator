...
def retrieve_documents(state: GraphState) -> GraphState:
    ...
    documents = retriever.invoke(topic) if topic else retriever.invoke("general overview of the document's main concepts")[:state['context_chunks']]
...
def fitb_agent(state: GraphState) -> GraphState:
    ...
    prompt = ChatPromptTemplate.from_template(
        ...
        **Example:** {{"questions": [{{"sentence": "An _________ has an 'equal' sign.", "correct_answer": "equation", "source_page": 1}}]}}
        **Context with Sources:** --- {context} ---"""
    )
...
def route_to_agent(state: GraphState) -> str:
    return f"{state['content_type'].lower()}_agent"
